...
import { Megaphone, Plus, Pause, Play, Archive, Trash2, ExternalLink } from "lucide-react";
...
const CampaignsPage = () => {
  ...
  const [newHiringType, setNewHiringType] = useState("FULL_TIME");
  ...
  const handleCreate = async () => {
    ...
    setActionError(null);
    try {
      await createCampaign({ name: newName.trim(), hiringType: newHiringType });
  ...
  return (
    <div className="space-y-6">
      ...
      {showNewForm && canManage && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-[#111827]">
          <input
            value={newName}
            onChange={(e) => setNewName(e.target.value)}
            placeholder="Campaign name..."
            className="min-w-[240px] flex-1 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm dark:border-slate-700 dark:bg-[#1E293B] dark:text-white"
          />
          <select
            value={newHiringType}
            onChange={(e) => setNewHiringType(e.target.value)}
            className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-medium dark:border-slate-700 dark:bg-[#1E293B] dark:text-white"
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <Link
                      to="/leads"
                      className="font-semibold text-slate-900 transition hover:text-violet-600 hover:underline dark:text-white dark:hover:text-violet-400"
                    ...
                    <Link
                      to="/leads"
                      className="text-slate-400 transition hover:text-violet-600 dark:hover:text-violet-400"
                      title="View leads"
                    >
