...
interface LeadDetailsProps {
  ...
  onScheduleMeeting: (lead: EnrichedLead) => void;
}

const LeadDetails = ({ selectedLead, onScheduleMeeting }: LeadDetailsProps) => {
  if (!selectedLead) {
    return (
      <div className="flex h-full min-h-[650px] flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-[#111827]">
        ...
        <h2 className="mt-6 text-xl font-semibold text-slate-900 dark:text-white">Lead Details</h2>
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-[#111827]">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-semibold text-slate-900 dark:text-white">{selectedLead.company}</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">{selectedLead.type}</p>
        </div>
      </div>

      <div className="mt-8 space-y-5">
        <InfoRow icon={<User className="h-5 w-5" />} label="Contact" value={selectedLead.contact} />
        <InfoRow icon={<Briefcase className="h-5 w-5" />} label="Designation" value={selectedLead.designation} />
        <InfoRow icon={<Mail className="h-5 w-5" />} label="Email" value={selectedLead.email} />
        <InfoRow icon={<Globe className="h-5 w-5" />} label="Website" value={selectedLead.website} />
        <InfoRow icon={<Target className="h-5 w-5" />} label="Lead Score" value={`${selectedLead.score}/100`} />
      </div>

      <div className="mt-8">
        <h3 className="mb-2 text-sm font-semibold text-slate-500 dark:text-slate-400">Current Status</h3>
